...
export const Login: React.FC = () => {
  ...
  React.useEffect(() => {
    if (auth.isAuthenticated) {
      if (auth.currentUser?.role === 'admin' || auth.currentUser?.role === 'repartidor' || auth.currentUser?.role === 'preparador') {
        navigate("/orders");
      } else {
        navigate("/");
      }
  ...
  return (
    <div className="min-h-screen bg-background-light flex items-center justify-center p-4">
      ...
      >
        {/* Logo */}
        <div className="flex flex-col items-center mb-10" onClick={() => navigate("/")}>
          <img
            src={logoImg}
            alt="Logo"
            className="w-35 h-35 object-contain mb-1"
          />
          <h1 className="text-3xl font-bold tracking-tight">PideBorolas</h1>
          <p className="text-slate-500 text-sm mt-1">Sistema de Entregas</p>
        </div>
        ...
        >
          <div>
            <h2 className="text-xl font-bold mb-1">Bienvenido de nuevo</h2>
